...
class OfficialIdGenerator
{
    public function generateIdCard($user)
    {
        ...
        $udetails = OfficialDetails::where('user_id', $user->id)->first();
...
$qrimage = QrCode::format('png')
    ->margin(1)
    ->merge(Storage::disk('s3')->url('BFI/photo/' . $udetails->photo), 0.3, true)
    ->size(500)
    ->errorCorrection('H')
    ->generate(json_encode($qrData)); // Encode the data as JSON

$s3Path = 'BFI/qrcode/' . $qrname;
...
return $s3Path;
    ...
    protected function generateFrontIdCard($user, $udetails)
    {
        ...
        $overlay = Image::make(Storage::disk('s3')->url('BFI/photo/' . $udetails->photo));
        ...
        $fronts3path = 'BFI/idfront/' . $user->username. '_front.jpg';
...
foreach ($textLines as $line) {
    ...
    $y += $fontSize + 5; // Adjust the spacing as needed
        ...
        if($user->qrcode == ''){
            ...
            $qrpath = Storage::disk('s3')->url('BFI/qrcode/' . $user->qrcode);
        }
    ...
    $backs3path = 'BFI/idback/' . $user->username. '_back.jpg';
